"""
Vision pipeline: Phase 1 detection + Phase 2 inference.
"""
...
class VisionPipeline:
    """
    Camera/Demo I/O and inference run on separate daemon threads.

    Queues:
      - frame_queue: raw frames -> UI
      - infer_queue: frames -> inference
      - result_queue: inference results -> UI
    """
    ...
    def start_live_camera(self) -> None:
        self._start(mode="live")

    def start_demo(self, demo_dir: str = DEMO_ASSETS_DIR) -> None:
        self._start(mode="demo", demo_dir=demo_dir)
    ...
    def enqueue_for_inference(self, frame_bgr) -> None:
        """Called by UI after display to feed inference queue."""
    ...
    # ── Image analytics mode ───────────────────────────────────────────

    def analyze_image(self, image_path: str) -> dict:
        """
        Image Upload & Analytics mode.
        Returns dict with: result, annotated_bgr, heatmap_bgr.
        """
        ...
        # Phase 1 detection for ROI (largest box)
        boxes = self._detect(bgr, bgr.shape[1], bgr.shape[0])
        if boxes:
            x, y, w, h = max(boxes, key=lambda b: b[2] * b[3])
            x1 = max(0, x - 8); y1 = max(0, y - 8)
            x2 = min(bgr.shape[1], x + w + 8)
        ...
        # Resize to CNN input size if available
        if self.clf.cnn is not None:
            inp = self.clf.cnn.get_input_details()
            sz = inp[0]["shape"][1:3]
            crop_rgb = cv2.resize(crop_rgb, (sz[1], sz[0]))

        result = self.clf.classify(crop_rgb)

        # Heatmap (Sobel magnitude) overlay
        ...
        heatmap = cv2.addWeighted(bgr, 0.6, heat, 0.4, 0)

        # Annotate ROI box
        annotated = bgr.copy()
        cv2.rectangle(annotated, (x1, y1), (x2, y2), (0, 230, 118), 2)
    ...
    def _start(self, mode: str, demo_dir: str = None) -> None:
        if self._running:
            return
        ...
        if mode == "live":
            self._io_thread = threading.Thread(target=self._camera_loop, daemon=True)
        else:
            self._io_thread = threading.Thread(target=self._demo_loop, args=(demo_dir,), daemon=True)
    ...
    def _demo_loop(self, demo_dir: str) -> None:
        if demo_dir is None:
            demo_dir = DEMO_ASSETS_DIR
        if not os.path.isdir(demo_dir):
            self._push_status(f"Demo assets not found: {demo_dir}")
            self._running = False
            return
        ...
        if not images:
            self._push_status("Demo assets folder is empty")
            self._running = False
            return
    ...
    def _inference_loop(self) -> None:
        target = 0.1  # simulate <100ms processing budget
        while self._running:
            t0 = time.time()
            try:
                frame = self.infer_queue.get(timeout=0.5)
            except queue.Empty:
                _safe_sleep(t0, target)
                continue
